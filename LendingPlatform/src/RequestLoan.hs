...
requestValidator contractInfo@ContractInfo{..} dat lenderTn sContext = validate
  where
    ...
    borrowerGetsWhatHeWants =
      assetClassValueOf (U.valuePaidToAddress sContext (borrowersAddress dat)) (loan dat)
      == loanAmnt dat

    ownHashFilter :: Maybe ValidatorHash -> Bool
    ownHashFilter mvh = Just (ownHash sContext) == mvh

    txHasOneRequestInputOnly :: Bool
    txHasOneRequestInputOnly = length (filter ownHashFilter $ toValidatorHash . txOutAddress . txInInfoResolved <$> txInfoInputs (U.info sContext)) == 1

    txHasOneScInputOnly :: Bool
    txHasOneScInputOnly =
      length (filter isJust $ toValidatorHash . txOutAddress . txInInfoResolved <$> txInfoInputs (U.info sContext)) == 1

    validateMint :: Bool
    validateMint = case U.mintFlattened sContext of
    ...
    validateBorrowerMint = case U.mintFlattened sContext of
    ...
    validateExpiration = after (requestExpiration dat) (U.range sContext)
    ...
    containsNewDatum txo = case U.getUpperBound sContext of
      Just ub -> findDatumHash' (expectedNewDatum ub) (U.info sContext) == txOutDatumHash txo
      Nothing -> False
    ...
    validateTxOuts :: Bool
    validateTxOuts = any txOutValidate (txInfoOutputs $ U.info sContext)
